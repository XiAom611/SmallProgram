...
def initmap():
	global raw_map, handle_map, xmax, ymax, wall, dic_map
	for i in range(ymax):
	...
	for y in range(ymax-1,-1,-1):
		for x in range(xmax):
			...
			if step_switch1 == 0:
				...
				if x != xmax - 1:
					print (format(dic_map[(x,y)],"6.2f"), end="")
				else:
					# to print out in a correct form
					# plus an enter
					print (format(dic_map[(x,y)],"6.2f"), end="\n")
	...
	for i in range(len(neighbor)):
		...
		# otherwise
			...
			# if it is smaller , change the distance saved before
				...
				if middle_point + dis2nei < neighbor[i]:
					# if it's smaller than before 
					# change it
					neighbor[i] = middle_point + dis2nei
			else:
	...
	if step_switch1 == 0 and step_switch2 == 0:
		...
		while path[-1] != startpoint:
			...
			# find the shortest way from goal to start
				...
				#print (neighbor)	
				mindis = min(neighbor)
				minplace = neighbor.index(mindis)
				# make the path
				# 0 means the (x,y-1)
				if minplace == 0:
